...
bool dexParseOptData(const u1* data, size_t length, DexFile* pDexFile)
{
    ...
    if (!isValidPointer(pOpt, pOptStart, pOptEnd)) {
        LOGD("Bogus opt data start pointer");
        return false;
    }

    /* Make sure that the opt data length is a whole number of words. */
    if ((optLength & 3) != 0) {
        LOGD("Unaligned opt data area end");
        return false;
    }
     ...
     */
    if (optLength < 8) {
        LOGD("Undersized opt data area (%u)", optLength);
        return false;
    }
    ...
    while (*pOpt != kDexChunkEnd) {
        if (!isValidPointer(pOpt + 2, pOptStart, pOptEnd)) {
            ...
            LOGD("Bogus opt data content pointer at offset %u", offset);
            return false;
        }
        ...
        if (!isValidPointer(pNextOpt, pOptStart, pOptEnd)) {
            const u4 offset = ((const u1*) pOpt) - data;
            LOGD("Opt data area problem for chunk of size %u at offset %u", size, offset);
            return false;
        }
        ...
        case kDexChunkRegisterMaps:
            LOGD("+++ found register maps, size=%u", size);
            pDexFile->pRegisterMapPool = pOptData;
            break;
        default:
            LOGD("Unknown chunk 0x%08x (%c%c%c%c), size=%d in opt data area",
                *pOpt,
