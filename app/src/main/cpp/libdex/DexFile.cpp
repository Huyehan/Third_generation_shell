...
DexClassLookup* dexCreateClassLookup(DexFile* pDexFile)
{
    ...
    LOGD("Class lookup: classes=%d slots=%d (%d%% occ) alloc=%d"
         " total=%d max=%d",
...
DexFile* dexFileParse(const u1* data, size_t length, int flags)
{
    ...
    if (length < sizeof(DexHeader)) {
        LOGD("too short to be a valid .dex");
    ...
    if (memcmp(data, DEX_OPT_MAGIC, 4) == 0) {
        ...
        if (memcmp(magic+4, DEX_OPT_MAGIC_VERS, 4) != 0) {
            LOGD("bad opt version (0x%02x %02x %02x %02x)",
                 magic[4], magic[5], magic[6], magic[7]);
            goto bail;
        }

        pDexFile->pOptHeader = (const DexOptHeader*) data;
        LOGD("Good opt header, DEX offset is %d, flags=0x%02x",
        ...
        if (pDexFile->pOptHeader->dexLength > length) {
            LOGD("File truncated? stored len=%d, rem len=%d",
...
//            LOGD("ERROR: bad checksum (%08x vs %08x)",
//                adler, pHeader->checksum);
//            if (!(flags & kDexParseContinueOnError))
//                goto bail;
//        } else {
//            LOGD("+++ adler32 checksum (%08x) verified", adler);
//        }
//
...
//                LOGD("ERROR: bad opt checksum (%08x vs %08x)",
//                    adler, pOptHeader->checksum);
//                if (!(flags & kDexParseContinueOnError))
//                    goto bail;
//            } else {
//                LOGD("+++ adler32 opt checksum (%08x) verified", adler);
//            }
//        }
//    }
    ...
    if (kVerifySignature) {
        ...
        if (memcmp(sha1Digest, pHeader->signature, kSHA1DigestLen) != 0) {
            ...
            LOGD("ERROR: bad SHA1 digest (%s vs %s)",
                dexSHA1DigestToStr(sha1Digest, tmpBuf1),
                dexSHA1DigestToStr(pHeader->signature, tmpBuf2));
            if (!(flags & kDexParseContinueOnError))
                goto bail;
        } else {
            LOGD("+++ sha1 digest verified");
        }
    }

    if (pHeader->fileSize != length) {
        LOGD("ERROR: stored file size (%d) != expected (%d)",
    ...
    if (pHeader->classDefsSize == 0) {
        LOGD("ERROR: DEX file has no classes in it, failing");
        goto bail;
    }
